...
- メンバーカラーによる歌詞の色分け表示（複数パート対応・グラデーション）
- タイムスタンプ連動の自動再生 Auto（requestAnimationFrame）
- ブロック単位での手動送り（◀ / ▶ ボタン・キーボード矢印）
- 横表示：スライド形式（現在ブロック＋次ブロックプレビュー）
- 縦表示：スクロール形式（全ブロック一覧・タップでフォーカス・自動スクロール）
- フルスクリーン対応
- タイムスタンプなし歌詞では Auto ボタン非表示

### 楽曲追加
- LRCLIB検索（曲名で検索→歌詞データを自動取得）
- 手動入力（曲名・アーティスト）
- 作成時にデフォルト2名のメンバーを自動登録

### 楽曲編集（4タブ）
1. **楽曲情報** — 曲名・アーティスト編集、メンバー登録（最大10名・カラーパレット・カスタムカラー）、保存ボタンで確定（UPSERT方式でIDを保持）
2. **歌詞編集** — LRC形式またはプレーンテキスト形式に対応、直接編集・LRCファイルインポート、保存時にパート分けを引き継ぎ
