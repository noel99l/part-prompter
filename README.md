...
| URL | 認証 | 説明 |
|---|---|---|
| `/prompter` | 不要 | 歌詞分け一覧・検索 |
| `/prompter/[songId]/detail` | 不要 | 楽曲詳細・歌詞分け閲覧 |
| `/prompter/[songId]` | 不要 | プロンプター表示 |
| `/prompter/playlist/[id]` | 不要 | プレイリストプロンプター |
| `/how-to-use` | 不要 | 使い方 |
| `/privacy` | 不要 | プライバシーポリシー |
| `/admin` | 必要 | 管理トップ |
| `/admin/songs` | 必要 | 楽曲管理（自分の楽曲のみ） |
...
- ハンバーガーメニューから管理画面ログイン

### 楽曲詳細（/prompter/[songId]/detail）
- 作者・パートメンバー・歌詞分けの閲覧
- 歌詞分けテキストのコピー（(A)歌詞... 形式）
...
1. **楽曲情報** — 曲名・アーティストのインライン編集、メンバー登録（最大10名・カラーパレット・カスタムカラー・即時保存）、公開/非公開トグル
...
4. **出力** — PPTX出力・PDF出力・テキストコピー

### プレイリスト
- プレイリスト作成・編集
- 曲名・アーティスト・歌詞で横断検索して追加（他ユーザーの楽曲も追加可能）
- ドラッグ&ドロップ並び替え
- プレイリスト単位でのプロンプター表示
